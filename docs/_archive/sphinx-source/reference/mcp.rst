...
The **@graphhunter/mcp** package (under ``platform/mcp/``) is an **MCP (Model Context Protocol) server** that exposes these operations as **tools** for AI assistants (e.g. Cursor, Claude Code). You can ask the AI to hunt for malicious or suspicious paths, expand nodes, or add notes while the app holds the session and graph. The older ``graph-hunter-mcp/`` package is retained on disk for one release cycle for operators with the 1.3.0 binary pinned; new setups should target ``platform/mcp/``.
...
Example **mcp.json** (path to ``platform/mcp`` can be absolute or relative to the project that uses it). Set ``GRAPHHUNTER_API_TOKEN`` to the value printed by the app at startup (required; otherwise you get 401 Unauthorized):
...
.. code-block:: json

   {
     "mcpServers": {
       "graph-hunter": {
         "command": "node",
         "args": ["C:/path/to/GraphHunter/platform/mcp/dist/src/index.js"],
         "env": {
...
- **New tools don't appear after upgrading Graph Hunter** — The MCP client caches the tool list when it first connects. After rebuilding ``@graphhunter/mcp`` (``npm run build`` under ``platform/mcp``) or updating the Graph Hunter app, **restart your MCP client** (Cursor / Claude Code / Desktop) so it picks up new tool registrations. A simple reload of the MCP connection is enough; no config change needed. Tools added in v1.3: ``get_relation_schema``, ``get_catalog_with_status``, ``explain_score``, ``tag_entity``, ``untag_entity``, ``list_tagged``, ``export_iocs``.

For more detail and checklist, see **platform/mcp/README.md** in the repository.
