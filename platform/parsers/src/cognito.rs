...
fn parse_notifications_csv(data: &str) -> Vec<ParsedTriple> {
    ...
    for line in lines {
        ...
        let user_name = extract_py_dict_value(metadata_str, "userName").unwrap_or_default();
        ...
        let ip_address = extract_py_dict_value(metadata_str, "ipAddress").unwrap_or_default();
        let risk_decision = extract_py_dict_value(metadata_str, "riskDecision").unwrap_or_default();
        let city = extract_py_dict_value(metadata_str, "city").unwrap_or_default();
        let country = extract_py_dict_value(metadata_str, "country").unwrap_or_default();
        let device_name = extract_py_dict_value(metadata_str, "deviceName").unwrap_or_default();
        let event_type = extract_py_dict_value(metadata_str, "eventType").unwrap_or_default();
        let challenges = extract_py_dict_value(metadata_str, "challenges").unwrap_or_default();
        let client_id = extract_py_dict_value(metadata_str, "clientId").unwrap_or_default();
        ...
        if !ip_address.is_empty() {
            ...
            let mut rel = Relation::new(&ip_address, &user_id, RelationType::Auth, timestamp);
        ...
        let mut rel2 = Relation::new(&user_id, &service_name, RelationType::Auth, timestamp);
...
fn parse_cloudwatch_json(data: &str) -> Vec<ParsedTriple> {
    ...
    for line in trimmed.lines() {
        let line = line
            ...
            .trim();
        if line.starts_with('{') {
            let json_str = if line.ends_with(',') {
                &line[..line.len() - 1]
            } else {
                line
            };
    ...
    for entry in &entries {
        let timestamp_ms = entry.get("timestamp").and_then(|v| v.as_i64()).unwrap_or(0);
        ...
        if let Some(json_start) = message.find("-> {") {
            ...
            if let Ok(payload) = serde_json::from_str::<Value>(json_str) {
                ...
                let service_entity = Entity::new("cognito-pool", EntityType::Service);
                let mut rel =
                    Relation::new(&user_name, "cognito-pool", RelationType::Auth, timestamp);
        ...
        if let Some(user_id) = extract_status_user(message) {
            ...
            let user_entity = Entity::new(&user_id, EntityType::User);
            let service_entity = Entity::new("cognito-pool", EntityType::Service);
            let mut rel = Relation::new(&user_id, "cognito-pool", RelationType::Auth, timestamp);
            rel.metadata = rel_meta;
            triples.push((user_entity, rel, service_entity));
        }
    }
...
mod tests {
    ...
    fn test_extract_py_dict_value() {
        ...
        assert_eq!(extract_py_dict_value(d, "userName"), Some("abc-123".into()));
        assert_eq!(
    ...
    fn test_extract_py_dict_value_none() {
        ...
        assert_eq!(extract_py_dict_value(d, "city"), Some("Lima".into()));
    }
    ...
    fn test_looks_like_cognito() {
        assert!(looks_like_cognito(r#"{"userPoolId":"us-east-1_abc"}"#));
