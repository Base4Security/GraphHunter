...
function summarizeResult(tool, result) {
    ...
    if (typeof o === "object") {
        if (typeof o.entity_types === "object" && Array.isArray(o.entity_types))
            return `types=${o.entity_types.length}`;
        if (typeof o.entities === "object" && Array.isArray(o.entities))
            return `entities=${o.entities.length}`;
        if (typeof o.edges === "object" && Array.isArray(o.edges))
...
const CONNECTION_ERROR_MSG = `GraphHunter app unreachable at ${API_BASE}. Ensure: (1) GraphHunter desktop app is running, (2) a session is loaded, (3) data has been ingested so the graph is non-empty.`;
const NOT_GRAPHHUNTER_MSG = `The server at ${API_BASE} responded but did not return JSON (e.g. it returned HTML). Another app may be using this port. Use only the GraphHunter desktop app on this port, or set GRAPHHUNTER_API_URL to the correct URL.`;
...
async function apiPost(path, body) {
    ...
    const bodySummary = body && typeof body === "object" && !Array.isArray(body)
        ? Object.fromEntries(Object.entries(body).map(([k, v]) => [k, Array.isArray(v) ? `[${v.length}]` : truncate(String(v), 40)]))
        : {};
...
server.tool("check_connection", "Check if the GraphHunter desktop app is reachable and whether a session is loaded. Call this first before other tools to avoid timeouts or unclear errors.", async () => {
    const start = Date.now();
    log("tool", "check_connection", {});
    try {
        const v = await apiGet("/health");
        const summary = typeof v === "object" && v !== null && "session_loaded" in v ? { session_loaded: v.session_loaded } : {};
...
server.tool("get_entity_types", "List entity types present in the current graph (e.g. IP, Host, User, Process).", async () => {
    const start = Date.now();
...
server.tool("search_entities", "Search entities by substring. Use to find nodes by ID, name, or metadata. Optional type filter (e.g. IP, Host).", {
...
server.tool("expand_node", "Expand from a node to get its neighbors (nodes and edges). Use to explore the graph from a starting node and find suspicious paths.", {
...
server.tool("get_node_details", "Get detailed information about a node: scores, degrees, neighbor counts, time range.", {
...
server.tool("get_subgraph", "Get the subgraph (nodes and edges) for a list of node IDs.", {
...
server.tool("get_events_for_node", "Get all relations/events for a node (incoming and outgoing edges).", {
    node_id: z.string().describe("Entity ID"),
}, async ({ node_id }) => {
    const start = Date.now();
...
server.tool("create_note", "Add a note to the current GraphHunter session. Use for saving hunt reports or findings. Optional node_id links the note to a graph node.", {
