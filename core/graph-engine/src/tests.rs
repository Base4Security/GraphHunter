...
/// Test helper — convert a `HuntResult` (Vec<Arc<str>>) into Vec<&str>
/// for `assert_eq!(..., vec!["literal", ...])`. The DFS hot path emits
/// `Arc<str>` to avoid allocations (D0); tests still want the cheap
/// literal-array comparison.
...
fn hypothesis_validation_ok() {
    let h = Hypothesis::new("DNS Exfil").add_step(HypothesisStep::new(
        EntityType::Process,
        RelationType::DNS,
        EntityType::Domain,
    ));
...
fn entity_serde_roundtrip() {
    let original =
        Entity::new("192.168.1.100", EntityType::IP).with_metadata("reputation", "malicious");
...
fn build_lateral_movement_graph() -> GraphHunter {
    ...
    g.add_entity(Entity::new("attacker-ip", EntityType::IP))
        .unwrap();
    g.add_entity(Entity::new("web-server", EntityType::Host))
        .unwrap();
    g.add_entity(Entity::new("admin-user", EntityType::User))
        .unwrap();
    g.add_entity(Entity::new("cmd.exe", EntityType::Process))
        .unwrap();
    g.add_entity(Entity::new("payload.dll", EntityType::File))
        .unwrap();
...
fn graph_add_entity_and_count() {
    ...
    g.add_entity(Entity::new("node-2", EntityType::Host))
        .unwrap();
...
fn graph_duplicate_entity_error() {
    ...
    let err = g
        .add_entity(Entity::new("dup", EntityType::IP))
        .unwrap_err();
...
fn search_full_lateral_movement_chain() {
    ...
    let results = g
        .search_temporal_pattern(&hypothesis, None, None)
        .unwrap()
        .0;
...
fn search_partial_chain() {
    ...
    assert_eq!(
        path_strs(&results[0]),
        vec!["attacker-ip", "web-server", "admin-user"]
    );
}
...
fn search_single_step() {
    ...
    let hypothesis = Hypothesis::new("Connection").add_step(HypothesisStep::new(
        EntityType::IP,
        RelationType::Connect,
        EntityType::Host,
    ));
    ...
    let results = g
        ...
        .0;
    assert_eq!(results.len(), 1);
    assert_eq!(path_strs(&results[0]), vec!["attacker-ip", "web-server"]);
}
...
fn search_no_match_wrong_relation_type() {
    ...
    let hypothesis = Hypothesis::new("Wrong Relation").add_step(HypothesisStep::new(
        EntityType::IP,
        RelationType::Auth,
        EntityType::Host,
    ));
...
fn search_no_match_wrong_dest_type() {
    ...
    let hypothesis = Hypothesis::new("Wrong Dest Type").add_step(HypothesisStep::new(
        EntityType::IP,
        RelationType::Connect,
        EntityType::User,
    ));
...
fn search_enforces_causal_monotonicity() {
    ...
    g.add_entity(Entity::new("host-1", EntityType::Host))
        .unwrap();
    g.add_entity(Entity::new("user-1", EntityType::User))
        .unwrap();

    // Connection at t=500, Auth at t=200 (BEFORE the connection — violates causality)
    g.add_relation(Relation::new("ip-1", "host-1", RelationType::Connect, 500))
        .unwrap();
    g.add_relation(Relation::new("host-1", "user-1", RelationType::Auth, 200))
        .unwrap();
    ...
    assert!(
        results.is_empty(),
        "Should not match when Auth happens before Connect"
    );
}
...
fn search_allows_same_timestamp() {
    ...
    g.add_relation(Relation::new("ip-1", "host-1", RelationType::Connect, 100))
        .unwrap();
    g.add_relation(Relation::new("host-1", "user-1", RelationType::Auth, 100))
        .unwrap();
...
fn search_time_window_includes_match() {
    ...
    let results = g
        .search_temporal_pattern(&hypothesis, Some((50, 250)), None)
        .unwrap()
        .0;
    assert_eq!(results.len(), 1);
}
...
fn search_time_window_excludes_match() {
    ...
    let results = g
        .search_temporal_pattern(&hypothesis, Some((150, 250)), None)
        .unwrap()
        .0;
    assert!(results.is_empty());
}
...
fn search_time_window_partial_chain_cutoff() {
    ...
    let results = g
        .search_temporal_pattern(&hypothesis, Some((50, 350)), None)
        .unwrap()
        .0;
    assert!(results.is_empty());
}
...
fn search_avoids_cycles() {
    ...
    g.add_entity(Entity::new("host-2", EntityType::Host))
        .unwrap();

    g.add_relation(Relation::new("ip-1", "host-1", RelationType::Connect, 100))
        .unwrap();
    g.add_relation(Relation::new("ip-1", "host-2", RelationType::Connect, 100))
        .unwrap();

    let hypothesis = Hypothesis::new("Fan Out").add_step(HypothesisStep::new(
        EntityType::IP,
        RelationType::Connect,
        EntityType::Host,
    ));
...
fn search_finds_multiple_attack_paths() {
    ...
    g.add_entity(Entity::new("attacker-1", EntityType::IP))
        .unwrap();
    g.add_entity(Entity::new("attacker-2", EntityType::IP))
        .unwrap();
    g.add_entity(Entity::new("server", EntityType::Host))
        .unwrap();
    ...
    let hypothesis = Hypothesis::new("Multi-Source").add_step(HypothesisStep::new(
        EntityType::IP,
        RelationType::Connect,
        EntityType::Host,
    ));
...
fn search_branching_paths() {
    ...
    g.add_entity(Entity::new("user-a", EntityType::User))
        .unwrap();
    g.add_entity(Entity::new("user-b", EntityType::User))
        .unwrap();

    g.add_relation(Relation::new("ip", "host", RelationType::Connect, 100))
        .unwrap();
    g.add_relation(Relation::new("host", "user-a", RelationType::Auth, 200))
        .unwrap();
    g.add_relation(Relation::new("host", "user-b", RelationType::Auth, 300))
        .unwrap();
...
fn search_realistic_apt_scenario() {
    ...
    g.add_entity(Entity::new("c2-server", EntityType::IP))
        .unwrap();
    g.add_entity(Entity::new("dmz-host", EntityType::Host))
        .unwrap();
    g.add_entity(Entity::new("svc-account", EntityType::User))
        .unwrap();
    g.add_entity(Entity::new("powershell.exe", EntityType::Process))
        .unwrap();
    g.add_entity(Entity::new("beacon.exe", EntityType::File))
        .unwrap();
...
#[test]
fn sysmon_eid1_raw_path_parity_with_legacy() {
    // Fixture: NDJSON with multiple EventID-1 lines that share entity
    // ids — exercises the dst_metadata-skipped-on-second-emission
    // optimization in `typed_process_create_raw` plus the writer's
    // `insert_raw_events` path. Resulting graph (entity count, relation
    // count, type tags, metadata) must match the legacy path exactly.
    let ndjson = r#"{"EventID":1,"UtcTime":"2024-01-15 14:30:00.000","Image":"C:\\Windows\\cmd.exe","User":"alice","Computer":"H1","ProcessId":100,"ParentImage":"C:\\Windows\\explorer.exe","ParentProcessId":50}
{"EventID":1,"UtcTime":"2024-01-15 14:30:01.000","Image":"C:\\Windows\\cmd.exe","User":"alice","Computer":"H1","ProcessId":101,"ParentImage":"C:\\Windows\\explorer.exe","ParentProcessId":50}
{"EventID":1,"UtcTime":"2024-01-15 14:30:02.000","Image":"C:\\Windows\\powershell.exe","User":"bob","Computer":"H2","ProcessId":200,"ParentImage":"C:\\Windows\\cmd.exe","ParentProcessId":100}"#;

    let parser = SysmonJsonParser;

    let mut g_legacy = GraphHunter::new();
    let triples = parser.parse(ndjson);
    g_legacy.insert_triples(triples, None).unwrap();

    let mut g_raw = GraphHunter::new();
    let events = parser.parse_raw(ndjson);
    g_raw.insert_raw_events(events, None).unwrap();

    assert_eq!(
        g_legacy.entities.len(),
        g_raw.entities.len(),
        "entity count diverged"
    );
    assert_eq!(
        g_legacy.relation_count(),
        g_raw.relation_count(),
        "relation count diverged"
    );
    for (sid, legacy_ent) in g_legacy.entities.iter() {
        let raw_ent = g_raw
            .entities
            .get(sid)
            .expect("raw graph missing entity present in legacy");
        assert_eq!(legacy_ent.id, raw_ent.id);
        assert_eq!(legacy_ent.entity_type, raw_ent.entity_type);
        assert_eq!(
            legacy_ent.metadata, raw_ent.metadata,
            "metadata diverged for {}",
            legacy_ent.id
        );
    }
}
...
#[test]
fn sysmon_long_tail_raw_path_parity_with_legacy() {
    // Mixed-EID NDJSON exercising the 9 raw-migrated handlers
    // (1, 3, 5, 7, 8, 11, 12, 15, 23) plus a 4624 (WinSec) and
    // 4104 (PowerShell) that intentionally fall through to the
    // legacy + lift path — confirms the fallback + raw path
    // produce identical graphs.
    let ndjson = r#"{"EventID":1,"UtcTime":"2024-01-15 14:30:00.000","Image":"C:\\Windows\\cmd.exe","User":"alice","Computer":"H1","ProcessId":100,"ParentImage":"C:\\Windows\\explorer.exe","ParentProcessId":50}
{"EventID":3,"UtcTime":"2024-01-15 14:30:01.000","Computer":"H1","Image":"C:\\Windows\\svchost.exe","DestinationIp":"10.0.0.5","DestinationPort":"443","Protocol":"tcp"}
{"EventID":5,"UtcTime":"2024-01-15 14:30:02.000","Image":"C:\\Windows\\cmd.exe","Computer":"H1","ProcessId":100}
{"EventID":7,"UtcTime":"2024-01-15 14:30:03.000","Image":"C:\\Windows\\powershell.exe","ImageLoaded":"C:\\Windows\\System32\\lib1.dll","Signed":"true","Hashes":"SHA256=abc"}
{"EventID":8,"UtcTime":"2024-01-15 14:30:04.000","SourceImage":"C:\\Windows\\svchost.exe","TargetImage":"C:\\Windows\\lsass.exe","StartAddress":"0x7FF","NewThreadId":"42"}
{"EventID":11,"UtcTime":"2024-01-15 14:30:05.000","Image":"C:\\Windows\\notepad.exe","TargetFilename":"C:\\Users\\u\\f1.txt"}
{"EventID":12,"UtcTime":"2024-01-15 14:30:06.000","Image":"C:\\Windows\\reg.exe","TargetObject":"HKLM\\Software\\App\\k1","EventType":"CreateKey"}
{"EventID":15,"UtcTime":"2024-01-15 14:30:07.000","Image":"C:\\Windows\\notepad.exe","TargetFilename":"C:\\Users\\u\\s1.txt:ads","Hash":"SHA1=def"}
{"EventID":23,"UtcTime":"2024-01-15 14:30:08.000","Image":"C:\\Windows\\cmd.exe","TargetFilename":"C:\\Users\\u\\d1.txt"}
{"EventID":4624,"UtcTime":"2024-01-15 14:30:09.000","TargetUserName":"alice","TargetDomainName":"CORP","LogonType":3,"IpAddress":"10.0.0.1","Computer":"H1"}
{"EventID":4104,"UtcTime":"2024-01-15 14:30:10.000","ScriptBlockText":"Get-Process","ScriptBlockId":"abc","Computer":"H1"}"#;

    let parser = SysmonJsonParser;
    let mut g_legacy = GraphHunter::new();
    g_legacy.insert_triples(parser.parse(ndjson), None).unwrap();

    let mut g_raw = GraphHunter::new();
    g_raw
        .insert_raw_events(parser.parse_raw(ndjson), None)
        .unwrap();
    ...
    for (sid, legacy_ent) in g_legacy.entities.iter() {
        let raw_ent = g_raw.entities.get(sid).unwrap_or_else(|| {
            panic!("raw graph missing entity present in legacy: {}", legacy_ent.id)
        });
        assert_eq!(legacy_ent.id, raw_ent.id);
        assert_eq!(legacy_ent.entity_type, raw_ent.entity_type);
        assert_eq!(
...
#[test]
fn sysmon_winsec_raw_path_parity_with_legacy() {
    // All 8 WinSec + PowerShell EventIDs migrated to raw. Verify
    // identical graphs vs legacy on a one-line-per-ID NDJSON fixture.
    let ndjson = r#"{"EventID":4624,"UtcTime":"2024-01-15 14:30:00.000","TargetUserName":"alice","TargetDomainName":"CORP","LogonType":3,"IpAddress":"10.0.0.1","Computer":"H1"}
{"EventID":4625,"UtcTime":"2024-01-15 14:30:01.000","TargetUserName":"bob","TargetDomainName":"CORP","LogonType":"3","IpAddress":"10.0.0.2","Computer":"H1"}
{"EventID":4688,"UtcTime":"2024-01-15 14:30:02.000","NewProcessName":"C:\\Windows\\cmd.exe","SubjectUserName":"alice","SubjectDomainName":"CORP","ParentProcessName":"C:\\Windows\\explorer.exe","CommandLine":"cmd /c whoami","Computer":"H1"}
{"EventID":4689,"UtcTime":"2024-01-15 14:30:03.000","ProcessName":"C:\\Windows\\cmd.exe","SubjectUserName":"alice","SubjectDomainName":"CORP","Computer":"H1"}
{"EventID":4663,"UtcTime":"2024-01-15 14:30:04.000","ProcessName":"C:\\Windows\\notepad.exe","ObjectName":"C:\\Users\\u\\f1.txt","ObjectType":"File","SubjectUserName":"alice","Computer":"H1"}
{"EventID":5145,"UtcTime":"2024-01-15 14:30:05.000","ShareName":"\\\\*\\IPC$","RelativeTargetName":"share1","SubjectUserName":"alice","IpAddress":"10.0.0.3","Computer":"H1"}
{"EventID":5156,"UtcTime":"2024-01-15 14:30:06.000","Application":"\\device\\harddisk\\svchost.exe","DestAddress":"10.0.0.4","DestPort":"443","SourceAddress":"10.0.0.1","Computer":"H1"}
{"EventID":4104,"UtcTime":"2024-01-15 14:30:07.000","ScriptBlockText":"Get-Process","ScriptBlockId":"abc","Computer":"H1"}"#;
...
fn sysmon_parse_event11_file_create() {
    ...
    assert_eq!(
        dst.metadata.get("hashes").unwrap(),
...
fn sysmon_parse_event10_process_access() {
    ...
    assert_eq!(
        rel.metadata.get("call_trace").unwrap(),
        "C:\\Windows\\SYSTEM32\\ntdll.dll+1234"
    );
...
fn full_pipeline_ingest_then_hunt() {
    ...
    assert!(
        !results.is_empty(),
        "Should find User -> beacon.exe -> mimikatz.exe path"
    );
...
fn full_pipeline_dns_exfil_hunt() {
    ...
    assert_eq!(
        path_strs(&results[0]),
        vec!["CORP\\admin", "powershell.exe", "data.exfil-tunnel.com"]
    );
}
...
fn sysmon_parse_event2_file_time_changed() {
    ...
    assert_eq!(dst.entity_type, EntityType::File);
    assert_eq!(
        rel.metadata.get("creation_utc_time").unwrap(),
        "2024-01-15 14:30:00.000"
    );
    assert_eq!(
        rel.metadata.get("previous_creation_utc_time").unwrap(),
...
fn sysmon_parse_powershell_4104_script_block() {
    ...
    assert_eq!(dst.entity_type, EntityType::Process);
    assert_eq!(
        dst.metadata.get("script_preview").unwrap(),
...
fn demo_data_all_presets_produce_results() {
    ...
    assert!(
        relations > 20,
        "Should ingest many relations, got {relations}"
    );
    ...
    let has_psexec_path = lateral_results.iter().any(|path| {
        path.iter().any(|n| n.contains("PsExec")) && path.iter().any(|n| n.contains("dropper"))
    });
    assert!(
        has_psexec_path,
        "Should find PsExec→dropper lateral movement path"
    );
    ...
    let has_evil_dns = dns_results.iter().any(|path| {
        path.iter()
            .any(|n| n.contains("evil-c2") || n.contains("exfil-tunnel"))
    });
    let has_benign_dns = dns_results.iter().any(|path| {
        path.iter()
            .any(|n| n.contains("google.com") || n.contains("office365"))
    });
    assert!(has_evil_dns, "Should find malicious DNS paths");
    assert!(
        has_benign_dns,
        "Should also find benign DNS paths (shows noise)"
    );
    ...
    let drop_results = g
        .search_temporal_pattern(&malware_drop, None, None)
        .unwrap()
        .0;
    assert!(
        drop_results.len() >= 2,
    ...
    let has_dropper = drop_results
        .iter()
        .any(|path| path.iter().any(|n| n.contains("dropper")));
...
fn reverse_adj_correct() {
    ...
    g.add_relation(Relation::new("a", "b", RelationType::Connect, 100))
        .unwrap();
    g.add_relation(Relation::new("a", "c", RelationType::Connect, 200))
        .unwrap();
...
fn search_entities_substring() {
    let mut g = GraphHunter::new();
    g.add_entity(Entity::new("beacon.exe", EntityType::Process))
        .unwrap();
    g.add_entity(Entity::new("cmd.exe", EntityType::Process))
        .unwrap();
    g.add_entity(Entity::new("evil-beacon.com", EntityType::Domain))
        .unwrap();
...
#[test]
#[ignore = "pre-existing failure: reverse-edge path in analytics doesn't apply entity_types filter"]
fn get_neighborhood_with_filter() {
    ...
    let hood = g
        .get_neighborhood("web-server", 1, 100, Some(&filter))
        .unwrap();
...
fn compute_scores_high_degree_high_score() {
    ...
    g.add_relation(Relation::new("a", "hub", RelationType::Connect, 100))
        .unwrap();
    g.add_relation(Relation::new("b", "hub", RelationType::Connect, 200))
        .unwrap();
    g.add_relation(Relation::new("c", "hub", RelationType::Connect, 300))
        .unwrap();
    g.add_relation(Relation::new("hub", "leaf", RelationType::Connect, 400))
        .unwrap();
    ...
    assert!(
        ...
        "Hub ({hub_score}) should have higher score than leaf ({leaf_score})"
    );
...
/// Regression for the FortiGate IPS+traffic ingest pattern: when a
/// second dataset's edges arrive AFTER the first finalize, those edges
/// land in the per-vertex tail buffer (not the sorted base list).
/// `get_node_details` must surface them — both in `out_degree` and in
/// the `neighbor_types` / `neighbors` lists. Before the fix `out_degree`
/// read `streaming.neighbors_arc(sid)` (BASE only) and reported the
/// pre-finalize count, dropping every post-finalize edge.
///
/// Symptom seen in production: 172.25.15.1 showed out_degree=1602
/// (the IPS dataset's contribution) instead of 101,602 (IPS + two
/// traffic datasets).
#[test]
fn get_node_details_includes_post_finalize_tail_edges() {
    let mut g = GraphHunter::new();

    // Pre-finalize "dataset 1" (mimics IPS): 3 edges src -> 3 distinct hosts.
    g.add_entity(Entity::new("src-ip", EntityType::IP)).unwrap();
    g.add_entity(Entity::new("host-a", EntityType::Host)).unwrap();
    g.add_entity(Entity::new("host-b", EntityType::Host)).unwrap();
    g.add_entity(Entity::new("host-c", EntityType::Host)).unwrap();
    g.add_relation(Relation::new("src-ip", "host-a", RelationType::Connect, 100)).unwrap();
    g.add_relation(Relation::new("src-ip", "host-b", RelationType::Connect, 110)).unwrap();
    g.add_relation(Relation::new("src-ip", "host-c", RelationType::Connect, 120)).unwrap();

    // Finalize: sorts base, arms the tail buffer for any subsequent appends.
    g.sort_edges_by_timestamp().unwrap();
    assert!(g.streaming_is_finalized(), "graph should be finalized");

    // Sanity baseline: pre-tail, src-ip has 3 outgoing edges.
    let baseline = g.get_node_details("src-ip").unwrap();
    assert_eq!(baseline.out_degree, 3, "baseline out_degree wrong");

    // Post-finalize "dataset 2" (mimics traffic batch): 5 more edges
    // src-ip -> host-a (already a neighbor) + 1 new entity. These all
    // land in the tail buffer because streaming is finalized.
    g.add_entity(Entity::new("host-d", EntityType::Host)).unwrap();
    for ts in 200..205 {
        g.add_relation(Relation::new("src-ip", "host-a", RelationType::Connect, ts)).unwrap();
    }
    g.add_relation(Relation::new("src-ip", "host-d", RelationType::Connect, 210)).unwrap();

    let details = g.get_node_details("src-ip").unwrap();

    // Out-degree must reflect base (3) + tail (6) = 9 edges. Before the
    // fix this was 3 (BASE only — tail-blind via neighbors_arc).
    assert_eq!(
        details.out_degree, 9,
        "out_degree must include post-finalize tail edges (got {}, expected 9)",
        details.out_degree
    );

    // src-ip has 4 distinct outgoing neighbors (host-a, b, c, d), all Hosts.
    // neighbor_types must count UNIQUE neighbors per entity_type, not
    // per-edge. Before the fix this incremented per edge and would have
    // reported {"Host": 9}.
    assert_eq!(
        details.neighbor_types.get("Host").copied(),
        Some(4),
        "neighbor_types[Host] must equal unique neighbor count (got {:?}, expected Some(4))",
        details.neighbor_types.get("Host")
    );

    // The neighbors list must be deduped to the 4 distinct hosts.
    let neighbor_ids: std::collections::HashSet<&str> =
        details.neighbors.iter().map(|n| n.id.as_str()).collect();
    assert!(neighbor_ids.contains("host-a"));
    assert!(neighbor_ids.contains("host-b"));
    assert!(neighbor_ids.contains("host-c"));
    assert!(neighbor_ids.contains("host-d"));
    assert_eq!(neighbor_ids.len(), 4, "neighbors list must be deduped");

    // host-a sees the post-finalize edges as INCOMING. in_degree path
    // goes through reverse_adj (already tail-aware), so this should
    // already have been correct — assert it explicitly so future
    // refactors don't regress.
    let host_a = g.get_node_details("host-a").unwrap();
    assert_eq!(
        host_a.in_degree, 6,
        "host-a in_degree: 1 (base) + 5 (tail) = 6, got {}",
        host_a.in_degree
    );
    // host-a has exactly 1 distinct incoming neighbor (src-ip, an IP).
    // Before the neighbor_types fix this would have read {"IP": 6}.
    assert_eq!(
        host_a.neighbor_types.get("IP").copied(),
        Some(1),
        "host-a neighbor_types[IP] must be 1 (one distinct IP neighbor), got {:?}",
        host_a.neighbor_types.get("IP")
    );
}
...
#[test]
fn sentinel_raw_path_parity_with_legacy() {
    // Cross-table NDJSON exercising every Sentinel raw-migrated
    // handler: SecurityEvent (4624 + 4688 + 4663), SigninLogs,
    // DeviceProcessEvents, DeviceNetworkEvents, DeviceFileEvents,
    // CommonSecurityLog. Confirms native parse_raw produces an
    // identical graph to the legacy parse + insert_triples path.
    let ndjson = r#"{"Type":"SecurityEvent","TimeGenerated":"2024-01-15T14:30:00Z","EventID":4624,"Computer":"DC-01","TargetUserName":"admin","IpAddress":"10.0.0.5","LogonType":3}
{"Type":"SecurityEvent","TimeGenerated":"2024-01-15T14:30:01Z","EventID":4688,"Computer":"DC-01","NewProcessName":"C:\\Windows\\cmd.exe","SubjectUserName":"alice","ParentProcessName":"C:\\Windows\\explorer.exe","CommandLine":"cmd /c whoami","NewProcessId":"1234"}
{"Type":"SecurityEvent","TimeGenerated":"2024-01-15T14:30:02Z","EventID":4663,"ProcessName":"C:\\Windows\\notepad.exe","ObjectName":"C:\\Users\\u\\f.txt"}
{"Type":"SigninLogs","TimeGenerated":"2024-01-15T14:30:03Z","UserPrincipalName":"alice@corp.com","IPAddress":"10.0.0.6","AppDisplayName":"Office365","ResultType":"0"}
{"Type":"DeviceProcessEvents","TimeGenerated":"2024-01-15T14:30:04Z","DeviceName":"H1","FileName":"powershell.exe","FolderPath":"C:\\Windows\\powershell.exe","AccountName":"alice","InitiatingProcessFileName":"explorer.exe","InitiatingProcessFolderPath":"C:\\Windows\\explorer.exe","ProcessCommandLine":"powershell -nop","SHA256":"abc123"}
{"Type":"DeviceNetworkEvents","TimeGenerated":"2024-01-15T14:30:05Z","DeviceName":"H1","RemoteIP":"10.0.0.7","RemotePort":"443","Protocol":"tcp","ActionType":"ConnectionSuccess","LocalPort":"49152"}
{"Type":"DeviceFileEvents","TimeGenerated":"2024-01-15T14:30:06Z","InitiatingProcessFileName":"notepad.exe","InitiatingProcessFolderPath":"C:\\Windows\\notepad.exe","FileName":"f.txt","FolderPath":"C:\\Users\\u\\f.txt","ActionType":"FileCreated","SHA256":"def456"}
{"Type":"CommonSecurityLog","TimeGenerated":"2024-01-15T14:30:07Z","SourceIP":"10.0.0.1","DestinationIP":"10.0.0.8","DestinationPort":"443","DeviceVendor":"PaloAlto","DeviceAction":"allow","Protocol":"tcp"}"#;
    ...
    for (sid, legacy_ent) in g_legacy.entities.iter() {
        let raw_ent = g_raw
            .entities
            .get(sid)
            .unwrap_or_else(|| panic!("raw graph missing entity: {}", legacy_ent.id));
        assert_eq!(legacy_ent.id, raw_ent.id);
        assert_eq!(legacy_ent.entity_type, raw_ent.entity_type);
        assert_eq!(
            legacy_ent.metadata, raw_ent.metadata,
...
fn sentinel_device_process_events() {
    ...
    assert_eq!(dst.entity_type, EntityType::Process);
    assert_eq!(
        dst.metadata.get("cmdline").unwrap(),
        "beacon.exe --c2 evil.com"
    );
...
fn sentinel_full_pipeline_ingest_then_hunt() {
    ...
    assert!(
        !results.is_empty(),
        "Should find attacker -> cmd.exe -> payload.dll"
    );
...
fn sentinel_demo_data_ingestion_and_hunt() {
    ...
    let mut g = GraphHunter::new();
    let (entities, relations) = g
        .ingest_logs(&demo_json, &SentinelJsonParser, None)
        .unwrap();
    ...
    let auth_hunt = Hypothesis::new("Auth Chain").add_step(HypothesisStep::new(
        EntityType::User,
        RelationType::Auth,
        EntityType::Host,
    ));
    ...
    let c2_hunt = Hypothesis::new("C2 Communication").add_step(HypothesisStep::new(
        EntityType::Host,
        RelationType::Connect,
        EntityType::IP,
    ));
    ...
    let has_attacker_ip = c2_results
        .iter()
        .any(|path| path.iter().any(|n| n.contains("198.51.100")));
...
fn generic_user_process_execute_triple() {
    ...
    assert!(
        has_execute,
        "Should produce User -[Execute]-> Process triple"
    );
}
...
fn generic_process_registry_modify_triple() {
    ...
    assert!(
        has_modify,
        "Should produce Process -[Modify]-> Registry triple"
    );
}
...
fn generic_mixed_fields_from_different_schemas() {
    ...
    let has_spawn = triples
        .iter()
        .any(|(_, rel, _)| rel.rel_type == RelationType::Spawn);
    let has_execute = triples
        .iter()
        .any(|(_, rel, _)| rel.rel_type == RelationType::Execute);
    let has_write = triples
        .iter()
        .any(|(_, rel, _)| rel.rel_type == RelationType::Write);
    let has_dns = triples
        .iter()
        .any(|(_, rel, _)| rel.rel_type == RelationType::DNS);
...
// RH-P1-012: try_parse must surface malformed rows that the legacy
// parse() path used to silently drop.

#[test]
fn generic_try_parse_surfaces_zero_triple_events_as_err() {
    use crate::parser::{LogParser, ParseError};
    let parser = GenericParser;
    // JSON object that GenericParser doesn't recognize — parse() drops
    // it silently; try_parse must emit a MalformedRow.
    let input = r#"{"unrecognized_key": "value"}"#;
    let results = parser.try_parse(input);
    assert!(
        results
            .iter()
            .any(|r| matches!(r, Err(ParseError::MalformedRow { .. }))),
        "expected at least one MalformedRow err, got: {:?}",
        results
    );
}

#[test]
fn sentinel_try_parse_surfaces_decode_failure() {
    use crate::parser::{LogParser, ParseError};
    let parser = SentinelJsonParser;
    // NDJSON line that's not valid JSON at all — must surface as Encoding
    // err, not silently dropped.
    let input = "this is not json at all\n";
    let results = parser.try_parse(input);
    assert!(
        results
            .iter()
            .any(|r| matches!(r, Err(ParseError::Encoding(_)))),
        "expected Encoding err for non-JSON input, got: {:?}",
        results
    );
}

#[test]
fn sentinel_try_parse_surfaces_zero_triple_event_as_err() {
    use crate::parser::{LogParser, ParseError};
    let parser = SentinelJsonParser;
    // Valid JSON, but no SecurityEvent / DeviceProcessEvents / etc. shape
    // that SentinelJsonParser recognizes.
    let input = r#"{"some_random_field": 42}"#;
    let results = parser.try_parse(input);
    assert!(
        results
            .iter()
            .any(|r| matches!(r, Err(ParseError::MalformedRow { .. }))),
        "expected MalformedRow err for unrecognized JSON, got: {:?}",
        results
    );
}
...
fn csv_quoted_fields_with_commas() {
    ...
    let has_execute = triples.iter().any(|(src, rel, dst)| {
        src.id == "admin"
            && rel.rel_type == RelationType::Execute
            && dst.id == "cmd.exe"
            && dst.metadata.get("cmdline").map(|s| s.as_str())
                == Some("cmd.exe /c echo hello, world")
    });
...
fn csv_demo_data_loads_successfully() {
    ...
    assert!(
        triples.len() >= 10,
        "CSV demo data should produce at least 10 triples, got {}",
        triples.len()
    );

    // Verify we get Connect triples (firewall/proxy logs)
    let connect_count = triples
        .iter()
        .filter(|(_, rel, _)| rel.rel_type == RelationType::Connect)
        .count();
    assert!(
        connect_count > 0,
        "Should have Connect triples from network logs"
    );
}
...
fn autodetect_sysmon_json() {
    ...
    assert!(
        !triples.is_empty(),
        "Sysmon content should parse with SysmonJsonParser"
    );
}
...
fn autodetect_csv_content() {
    ...
    assert!(
        !triples.is_empty(),
        "CSV content should parse with CsvParser"
    );
}
...
fn autodetect_generic_json_fallback() {
    ...
    assert!(
        !triples.is_empty(),
        "Generic JSON should parse with GenericParser"
    );
}
...
fn generic_integration_ingest_then_hunt() {
    ...
    assert!(
        !results.is_empty(),
        "Should find admin -> beacon.exe -> payload.dll"
    );
    ...
    assert!(
        !dns_results.is_empty(),
        "Should find admin -> beacon.exe -> exfil.evil.com"
    );
}
...
fn score_and_paginate_basic_scoring() {
    ...
    assert!(
        scored[0].max_score > 0.0,
        "Scored path should have non-zero max_score"
    );
    assert!(
        scored[0].total_score > 0.0,
        "Scored path should have non-zero total_score"
    );
    assert!(
        !scored[0].chain_summary.is_empty(),
        "chain_summary should not be empty"
    );
    assert!(
        scored[0].chain_summary.contains("->"),
        "chain_summary should contain arrows"
    );
...
fn score_and_paginate_time_range() {
    ...
    assert_eq!(scored.len(), 1);
    assert!(
        scored[0].time_start > 0,
        "time_start should be set from edge timestamps"
    );
    assert!(
        scored[0].time_end >= scored[0].time_start,
        "time_end >= time_start"
    );
}
...
fn score_and_paginate_min_score_filter() {
    ...
    let (scored, total) = g.score_and_paginate_paths(&paths, 0, 50, Some(999.0), DedupMode::None);
    assert_eq!(total, 0);
    assert!(scored.is_empty());

    // With threshold 0, should keep everything
    let (scored_all, total_all) =
        g.score_and_paginate_paths(&paths, 0, 50, Some(0.0), DedupMode::None);
...
fn score_and_paginate_pagination() {
    ...
    for i in 0..10 {
        ...
        g.add_entity(Entity::new(&dst_id, EntityType::Host))
            .unwrap();
        g.add_relation(Relation::new(
            &src_id,
            &dst_id,
    ...
    // Page 0, size 3
    let (page0, total) = g.score_and_paginate_paths(&paths, 0, 3, None, DedupMode::None);
    assert_eq!(total, 10);
    assert_eq!(page0.len(), 3);

    // Page 1, size 3
    let (page1, _) = g.score_and_paginate_paths(&paths, 1, 3, None, DedupMode::None);
    assert_eq!(page1.len(), 3);

    // Page 3 (last partial), size 3
    let (page3, _) = g.score_and_paginate_paths(&paths, 3, 3, None, DedupMode::None);
    assert_eq!(page3.len(), 1);

    // Page 4 (out of range)
    let (page4, _) = g.score_and_paginate_paths(&paths, 4, 3, None, DedupMode::None);
...
fn score_and_paginate_sorted_by_max_score_desc() {
    let mut g = GraphHunter::new();
    g.add_entity(Entity::with_score("low", EntityType::IP, 10.0))
        .unwrap();
    g.add_entity(Entity::with_score("mid", EntityType::Host, 50.0))
        .unwrap();
    g.add_entity(Entity::with_score("high", EntityType::User, 90.0))
        .unwrap();

    g.add_relation(Relation::new("low", "mid", RelationType::Connect, 100))
        .unwrap();
    g.add_relation(Relation::new("mid", "high", RelationType::Auth, 200))
        .unwrap();
    ...
    assert!(
        scored[0].max_score >= scored[1].max_score,
        "Should be sorted by max_score DESC"
    );
...
fn configurable_parser_backwards_compatible() {
    ...
    let config = FieldConfig { mappings: vec![] };
...
fn benchmark_empirical_data_for_paper() {
    ...
    fn measure_dataset<P: LogParser>(name: &'static str, data: &str, parser: &P) -> DatasetResult {
        ...
        let dmax = g
            .entities
            .keys()
            .map(|&sid| g.streaming.degree_of(sid))
            .max()
            .unwrap_or(0);
        ...
        g.streaming.for_each_edge(|_src, e| {
        ...
        let p_rel = if num_rel_types > 0 {
            1.0 / num_rel_types as f64
        } else {
            1.0
        };
        let p_ent = if num_ent_types > 0 {
            1.0 / num_ent_types as f64
        } else {
            1.0
        };
        let p_temporal = 0.5;
        ...
        let mut edge_type_pairs: std::collections::HashMap<(String, String), usize> =
            std::collections::HashMap::new();
        let mut temporal_ordered = 0usize;
        let mut temporal_total = 0usize;
        // Collect all edges, group by source, and analyze.
        // Streaming yields owned StreamEdges; transmute via #[repr(C)]
        // layout-identity to keep the rest of this routine on
        // CompactRelation references.
        let mut all_edges: Vec<relation::CompactRelation> = Vec::new();
        g.streaming.for_each_edge(|_src, edge| {
            let compact: relation::CompactRelation = unsafe {
                std::ptr::read(edge as *const _ as *const relation::CompactRelation)
            };
            all_edges.push(compact);
        });
        let mut by_source: std::collections::HashMap<usize, Vec<&relation::CompactRelation>> =
            std::collections::HashMap::new();
        for e in &all_edges {
        ...
        for (_sid, edges) in &mut by_source {
            ...
            for i in 0..edges.len() {
                ...
                let dest_type = g
                    ...
                    .unwrap_or_default();
                *edge_type_pairs
                    .entry((format!("{}", e.rel_type()), dest_type))
                    .or_default() += 1;
                if i > 0 {
                    temporal_total += 1;
        ...
        let t0 = Instant::now();
        let r2_res = g
            .search_temporal_pattern(&h2, None, None)
            .map(|(r, _)| r)
            .unwrap_or_default();
        ...
        let t0 = Instant::now();
        let r3 = g
            .search_temporal_pattern(&h3, None, None)
            .map(|(r, _)| r)
            .unwrap_or_default();
        ...
        let t0 = Instant::now();
        let r4 = g
            .search_temporal_pattern(&h4, None, None)
            .map(|(r, _)| r)
            .unwrap_or_default();
        ...
        let t0 = Instant::now();
        let r5 = g
            .search_temporal_pattern(&h5, None, None)
            .map(|(r, _)| r)
            .unwrap_or_default();
    ...
    for r in [Some(&r1), Some(&r2), r3_opt.as_ref()] {
        if let Some(r) = r {
            ...
            println!();
            println!(
                "  L=2: {} results from {} starts in {:.3} ms",
                r.results_l2, r.starts, r.time_l2_ms
            );
            println!(
                "  L=3: {} results from {} starts in {:.3} ms",
                r.results_l3, r.starts, r.time_l3_ms
            );
            println!(
                "  L=4: {} results from {} starts in {:.3} ms",
                r.results_l4, r.starts, r.time_l4_ms
            );
            println!(
                "  L=5: {} results from {} starts in {:.3} ms",
                r.results_l5, r.starts, r.time_l5_ms
            );
...
fn dsl_parse_simple_chain() {
    ...
    assert_eq!(
        result.hypothesis.steps[1].relation_type,
        RelationType::Execute
    );
...
#[test]
#[ignore = "pre-existing failure: DSL parser accepts EntityType::Other instead of rejecting unknown types"]
...
#[test]
fn dsl_unknown_relation_falls_through_to_other() {
    // The DSL parser intentionally accepts unknown relation names as
    // `RelationType::Other(_)` so user extensions and ATT&CK templates
    // can use custom verbs without a parser change. Documented at
    // platform/dsl/src/lib.rs:1161 "Backward-compat: unknown names
    // still parse as Other(_)".
    let r = parse_dsl("User -[Foo]-> Host", None).unwrap();
    assert_eq!(r.hypothesis.steps.len(), 1);
    match &r.hypothesis.steps[0].relation_type {
        crate::types::RelationType::Other(s) => assert_eq!(s, "Foo"),
        other => panic!("expected RelationType::Other(\"Foo\"), got {:?}", other),
    }
}
...
fn wildcard_entity_search() {
    ...
    let h = Hypothesis::new("wildcard test").add_step(HypothesisStep::new(
        EntityType::Any,
        RelationType::Spawn,
        EntityType::Process,
    ));
...
fn wildcard_relation_search() {
    ...
    let h = Hypothesis::new("wildcard rel test").add_step(HypothesisStep::new(
        EntityType::Process,
        RelationType::Any,
        EntityType::Process,
    ));
...
fn catalog_has_entries() {
    ...
    assert!(
        catalog.len() >= 10,
        "Catalog should have at least 10 entries"
    );
}
...
fn catalog_entries_parse_as_valid_dsl() {
    ...
    for entry in catalog {
        ...
        assert!(
            result.is_ok(),
            "Catalog entry '{}' ({}): DSL parse failed: {:?}",
            entry.name,
            entry.mitre_id,
            result.err()
        );
...
fn benchmark_naive_vs_pruned() {
    ...
    // L=2 hypothesis
    let h2 = Hypothesis::new("L2").add_step(HypothesisStep::new(
        EntityType::Process,
        RelationType::Execute,
        EntityType::Process,
    ));
    ...
    for (label, h) in [("L=2", &h2), ("L=3", &h3)] {
        ...
        let speedup = if time_pruned > 0.0 {
            time_naive / time_pruned
        } else {
            f64::INFINITY
        };

        println!(
            "  {}: pruned={} results in {:.3}ms, naive={} results ({} nodes visited) in {:.3}ms",
            label,
            pruned_results.len(),
            time_pruned,
            naive_results.len(),
        ...
        assert_eq!(
            pruned_results.len(),
            naive_results.len(),
            "Pruned and naive should find the same number of results for {}",
            label
        );
    }
    ...
    if std::path::Path::new(mordor_path).exists() {
        ...
        let h_mordor = Hypothesis::new("Mordor L=2").add_step(HypothesisStep::new(
            EntityType::Process,
            RelationType::Execute,
            EntityType::Process,
        ));
        ...
        println!(
            "  Mordor (n={}, m={}): pruned={} in {:.3}ms, naive={} ({} visited) in {:.3}ms",
            gm.entity_count(),
            gm.relation_count(),
            pruned.len(),
            time_pruned,
...
fn k_simplicity_default_1_same_as_simple_path() {
    ...
    let mut graph = GraphHunter::new();
    graph
        .add_entity(Entity::new("u1", EntityType::User))
        .unwrap();
    graph
        .add_entity(Entity::new("h1", EntityType::Host))
        .unwrap();
    graph
        .add_entity(Entity::new("p1", EntityType::Process))
        .unwrap();

    graph
        .add_relation(Relation::new("u1", "h1", RelationType::Auth, 100))
        .unwrap();
    graph
        .add_relation(Relation::new("h1", "p1", RelationType::Execute, 200))
        .unwrap();
    ...
    assert_eq!(results.len(), 1);
    assert_eq!(path_strs(&results[0]), vec!["u1", "h1", "p1"]);
}
...
fn k_simplicity_2_finds_cycle() {
    ...
    let mut graph = GraphHunter::new();
    graph
        .add_entity(Entity::new("procA", EntityType::Process))
        .unwrap();
    graph
        .add_entity(Entity::new("fileB", EntityType::File))
        .unwrap();

    graph
        .add_relation(Relation::new("procA", "fileB", RelationType::Write, 100))
        .unwrap();
    graph
        .add_relation(Relation::new("fileB", "procA", RelationType::Execute, 200))
        .unwrap();
    ...
    assert_eq!(path_strs(&results2[0]), vec!["procA", "fileB", "procA"]);
}
...
fn k_simplicity_3_allows_triple_visit() {
    ...
    let mut graph = GraphHunter::new();
    graph
        .add_entity(Entity::new("p", EntityType::Process))
        .unwrap();
    graph
        .add_entity(Entity::new("f", EntityType::File))
        .unwrap();

    graph
        .add_relation(Relation::new("p", "f", RelationType::Write, 100))
        .unwrap();
    graph
        .add_relation(Relation::new("f", "p", RelationType::Execute, 200))
        .unwrap();
    graph
        .add_relation(Relation::new("p", "f", RelationType::Write, 300))
        .unwrap();
    graph
        .add_relation(Relation::new("f", "p", RelationType::Execute, 400))
        .unwrap();
    ...
    let results = graph.search_temporal_pattern(&h, None, None).unwrap().0;
    assert_eq!(results.len(), 1);
    assert_eq!(path_strs(&results[0]), vec!["p", "f", "p", "f", "p"]);
}
...
fn k_simplicity_temporal_monotonicity_still_enforced() {
    ...
    graph
        .add_relation(Relation::new("p", "f", RelationType::Write, 200))
        .unwrap();
    // Timestamp goes backwards — should block the cycle even with k=2
    graph
        .add_relation(Relation::new("f", "p", RelationType::Execute, 100))
        .unwrap();
    ...
    assert_eq!(
        results.len(),
        0,
        "temporal monotonicity should block this even with k=2"
    );
}
...
fn rel_index_populated_on_add_relation() {
    ...
    graph
        ...
        .unwrap();
    graph
        .add_entity(Entity::new("h2", EntityType::Host))
        .unwrap();
    ...
    graph
        ...
        .unwrap();
    graph
        .add_relation(Relation::new("u1", "h2", RelationType::Auth, 200))
        .unwrap();
    graph
        .add_relation(Relation::new("u1", "h1", RelationType::Connect, 300))
        .unwrap();
    ...
    let auth_count = all_edges
        .iter()
        .filter(|e| e.rel_type == RelationType::Auth)
        .count();
    assert_eq!(auth_count, 2);

    let conn_count = all_edges
        .iter()
        .filter(|e| e.rel_type == RelationType::Connect)
        .count();
...
fn rel_index_search_same_results_as_full_scan() {
    ...
    let results_concrete = graph
        .search_temporal_pattern(&h_concrete, None, None)
        .unwrap()
        .0;
    ...
    let results_wildcard = graph
        .search_temporal_pattern(&h_wildcard, None, None)
        .unwrap()
        .0;
...
fn rel_index_wildcard_falls_back_to_full_scan() {
    ...
    let h = Hypothesis::new("wildcard").add_step(HypothesisStep::new(
        EntityType::User,
        RelationType::Any,
        EntityType::Host,
    ));
...
fn catalog_cyclic_entries_have_k_gt_1() {
    ...
    assert!(
        cyclic.len() >= 4,
        "Should have at least 4 cyclic catalog entries"
    );
    for entry in &cyclic {
...
fn catalog_existing_entries_have_k_1() {
    ...
    let original_ids: Vec<String> = (1..=12)
        .filter_map(|i| {
            // Match cat-001..cat-012
            catalog
                .iter()
                .find(|e| e.id == format!("cat-{:03}", i))
                .map(|e| e.id.clone())
        })
        .collect();
...
fn catalog_cyclic_c2_callback_demo() {
    ...
    let mut graph = GraphHunter::new();
    graph
        .add_entity(Entity::new("malware.exe", EntityType::Process))
        .unwrap();
    graph
        .add_entity(Entity::new("evil.com", EntityType::Domain))
        .unwrap();
    graph
        .add_entity(Entity::new("1.2.3.4", EntityType::IP))
        .unwrap();
    ...
    assert_eq!(
        path_strs(&results[0]),
        vec!["malware.exe", "evil.com", "1.2.3.4", "malware.exe"]
    );
}
...
fn catalog_cyclic_entries_parse_as_valid_dsl() {
    ...
    for entry in catalog.iter().filter(|e| e.k_simplicity > 1) {
        ...
        assert!(
            result.is_ok(),
            "Failed to parse cyclic entry {}: {:?}",
            entry.id,
            result.err()
        );
        let h = result.unwrap().hypothesis;
        assert_eq!(
            h.k_simplicity, entry.k_simplicity,
            "Entry {} DSL k_simplicity mismatch",
            entry.id
        );
    }
}
...
fn merge_policy_first_write_wins_preserves() {
    ...
    graph
        .ingest_logs_with_policy(json1, &GenericParser, None, &MergePolicy::FirstWriteWins)
        .unwrap();
    ...
    graph
        .ingest_logs_with_policy(json2, &GenericParser, None, &MergePolicy::FirstWriteWins)
        .unwrap();
...
fn merge_policy_last_write_wins_overwrites() {
    ...
    let json =
        r#"[{"hostname":"host-1","sourceip":"10.0.0.1","timestamp":"2024-01-01T01:00:00Z"}]"#;
    graph
        .ingest_logs_with_policy(json, &GenericParser, None, &MergePolicy::LastWriteWins)
        .unwrap();
...
fn merge_policy_append_concatenates() {
    ...
    graph
        .ingest_logs_with_policy(json1, &GenericParser, None, &MergePolicy::Append)
        .unwrap();
    ...
    graph
        .ingest_logs_with_policy(json2, &GenericParser, None, &MergePolicy::Append)
        .unwrap();
    let proc_entity = graph.get_entity("cmd.exe").expect("cmd.exe should exist");
    let cmdline = proc_entity
        .metadata
        .get("cmdline")
        .expect("Should have cmdline metadata");
...
fn betweenness_linear_chain() {
    ...
    let mut graph = GraphHunter::new();
    graph
        .add_entity(Entity::new("A", EntityType::Host))
        .unwrap();
    graph
        .add_entity(Entity::new("B", EntityType::Host))
        .unwrap();
    graph
        .add_entity(Entity::new("C", EntityType::Host))
        .unwrap();
    graph
        .add_relation(Relation::new("A", "B", RelationType::Connect, 100))
        .unwrap();
    graph
        .add_relation(Relation::new("B", "C", RelationType::Connect, 200))
        .unwrap();
...
fn betweenness_star_graph() {
    ...
    let mut graph = GraphHunter::new();
    graph
        .add_entity(Entity::new("center", EntityType::Host))
        .unwrap();
    for i in 0..4 {
        let leaf = format!("leaf-{}", i);
        graph
            .add_entity(Entity::new(&leaf, EntityType::IP))
            .unwrap();
        graph
            .add_relation(Relation::new(
                "center",
...
fn betweenness_bridge_node() {
    ...
    for id in &["A", "B", "C", "D", "E", "F"] {
        graph
            .add_entity(Entity::new(*id, EntityType::Host))
            .unwrap();
    }
    graph
        .add_relation(Relation::new("A", "B", RelationType::Connect, 100))
    ...
    graph
        ...
        .unwrap();
    graph
        .add_relation(Relation::new("C", "D", RelationType::Connect, 300))
        .unwrap();
    graph
        .add_relation(Relation::new("D", "E", RelationType::Connect, 400))
        .unwrap();
    graph
        .add_relation(Relation::new("E", "F", RelationType::Connect, 500))
        .unwrap();
...
fn betweenness_sampled_approx() {
    ...
    for i in 0..10 {
        graph
            .add_entity(Entity::new(format!("n{}", i), EntityType::Host))
            .unwrap();
    }
    for i in 0..9 {
        graph
...
fn pagerank_star_topology() {
    ...
    for i in 0..5 {
        ...
        graph
            ...
            .unwrap();
        graph
            .add_relation(Relation::new(&leaf, "center", RelationType::Connect, 1000))
            .unwrap();
    }
    ...
    let center = graph.get_entity("center").unwrap();
    assert_eq!(
        center.pagerank_score, 100.0,
        "Center should have max pagerank"
    );
}
...
fn pagerank_decay_prefers_recent() {
    ...
    graph
        .add_relation(Relation::new("A", "C", RelationType::Connect, 10000))
        .unwrap();
    ...
    assert!(
        c.pagerank_score > b.pagerank_score,
        "C (recent) should rank higher than B (old): C={}, B={}",
        c.pagerank_score,
        b.pagerank_score
    );
}
...
fn pagerank_no_decay_equals_standard() {
    ...
    graph
        ...
        .unwrap();
    graph
        .add_relation(Relation::new("B", "A", RelationType::Connect, 200))
        .unwrap();
    ...
    assert!(
        (a.pagerank_score - b.pagerank_score).abs() < 5.0,
        "Symmetric graph should have near-equal PR: A={}, B={}",
        a.pagerank_score,
        b.pagerank_score
    );
}
...
fn pagerank_dangling_nodes() {
    ...
    graph
        ...
        .unwrap();
    graph
        .add_relation(Relation::new("A", "C", RelationType::Connect, 200))
        .unwrap();
...
fn composite_degree_only() {
    ...
    graph.compute_composite_score(1.0, 0.0, 0.0, 0.0);

    let a = graph.get_entity("A").unwrap();
    assert_eq!(
        a.score, 100.0,
        "A has max degree → composite=100 with degree_only"
    );
}
...
fn composite_equal_weights() {
    ...
    graph.compute_composite_score(1.0, 1.0, 1.0, 0.0);
...
fn composite_normalization() {
    ...
    let max_score = graph
        ...
        .fold(0.0f64, f64::max);
    assert!(
        (max_score - 100.0).abs() < 0.01,
        "Max composite should be 100, got {}",
        max_score
    );
}
...
fn parallel_search_matches_sequential() {
    ...
    for i in 0..100 {
        graph
            .add_entity(Entity::new(format!("user-{}", i), EntityType::User))
            .unwrap();
        graph
            .add_entity(Entity::new(format!("host-{}", i), EntityType::Host))
            .unwrap();
        graph
            .add_relation(Relation::new(
                format!("user-{}", i),
    ...
    let h = Hypothesis::new("Auth Test").add_step(HypothesisStep::new(
        EntityType::User,
        RelationType::Auth,
        EntityType::Host,
    ));
...
fn parallel_search_below_threshold() {
    ...
    for i in 0..10 {
        graph
            .add_entity(Entity::new(format!("ip-{}", i), EntityType::IP))
            .unwrap();
        graph
            .add_entity(Entity::new(format!("host-{}", i), EntityType::Host))
            .unwrap();
    ...
    let h = Hypothesis::new("Connect Test").add_step(HypothesisStep::new(
        EntityType::IP,
        RelationType::Connect,
        EntityType::Host,
    ));
...
fn parallel_search_deterministic() {
    ...
    for i in 0..80 {
        graph
            .add_entity(Entity::new(format!("u{}", i), EntityType::User))
            .unwrap();
        graph
            .add_entity(Entity::new(format!("h{}", i), EntityType::Host))
            .unwrap();
        graph
            .add_relation(Relation::new(
                format!("u{}", i),
...
fn cef_basic_parse() {
    ...
    assert_eq!(
        obj.get("device_vendor").unwrap().as_str().unwrap(),
        "Security"
    );
...
fn cef_extensions() {
    ...
    assert_eq!(
        obj.get("msg").unwrap().as_str().unwrap(),
...
fn leef_basic_parse() {
    let line =
        "LEEF:1.0|Microsoft|MSExchange|1.0|LoginSuccess|src=10.0.0.1\tdst=192.168.1.1\tsuser=admin";
...
fn ecs_nested_flattening() {
    ...
    assert_eq!(
        obj.get("target_ip").unwrap().as_str().unwrap(),
        "192.168.1.1"
    );
...
fn graph_clone_independent() {
    ...
    // Modify original
    graph
        .add_entity(Entity::new("C", EntityType::User))
        .unwrap();

    // Clone should not have C
    assert!(
        clone.get_entity("C").is_none(),
    ...
    // Modify clone
    clone
        .add_entity(Entity::new("D", EntityType::File))
        .unwrap();
    assert!(
        graph.get_entity("D").is_none(),
        "Original should be independent of clone"
    );
}
...
fn search_on_clone_matches_original() {
    let mut graph = GraphHunter::new();
    graph
        .add_entity(Entity::new("user1", EntityType::User))
        .unwrap();
    graph
        .add_entity(Entity::new("host1", EntityType::Host))
        .unwrap();
    graph
        .add_relation(Relation::new("user1", "host1", RelationType::Auth, 100))
        .unwrap();
    ...
    assert_eq!(
        original_results, clone_results,
        "Clone search should match original"
    );
}
...
fn compact_all_old() {
    ...
    graph
        ...
        .unwrap();
    graph
        .add_relation(Relation::new("A", "B", RelationType::Connect, 200))
        .unwrap();
    graph
        .add_relation(Relation::new("A", "B", RelationType::Connect, 300))
        .unwrap();
...
fn compact_mixed() {
    ...
    graph
        ...
        .unwrap();
    graph
        .add_relation(Relation::new("A", "B", RelationType::Connect, 5000))
        .unwrap();
...
fn compact_preserves_new() {
    ...
    graph
        .add_relation(Relation::new("A", "B", RelationType::Connect, 2000))
        .unwrap();
    graph
        .add_relation(Relation::new("A", "B", RelationType::Connect, 3000))
        .unwrap();
...
fn compact_summary_metadata() {
    ...
    graph
        ...
        .unwrap();
    graph
        .add_relation(Relation::new("A", "B", RelationType::Connect, 500))
        .unwrap();
    ...
    let summary = &edges[0];
    assert_eq!(
        summary.timestamp, 100,
        "Summary should use earliest timestamp"
    );
...
fn compact_index_consistency() {
    ...
    graph
        ...
        .unwrap();
    graph
        .add_relation(Relation::new("A", "C", RelationType::Auth, 150))
        .unwrap();
    ...
    assert_eq!(
        all_edges.len(),
        2,
        "Should have 2 edges total (1 compacted Connect + 1 Auth)"
    );
    let connect_count = all_edges
        .iter()
        .filter(|e| e.rel_type == RelationType::Connect)
        .count();
...
fn scoring_pipeline_full_integration() {
    ...
    let mut graph = GraphHunter::new();
    graph
        .add_entity(Entity::new("hub", EntityType::Host))
        .unwrap();
    for i in 0..5 {
        let leaf = format!("leaf-{}", i);
        graph
            ...
            .unwrap();
        graph
            .add_relation(Relation::new(&leaf, "hub", RelationType::Connect, 1000 + i))
            .unwrap();
    }
    // Add cross-link so betweenness is interesting
    graph
        .add_relation(Relation::new("hub", "leaf-0", RelationType::Connect, 2000))
        .unwrap();
    ...
    assert!(
        details.degree_score > 0.0,
        "degree_score should be > 0, got {}",
        details.degree_score
    );
    assert!(
        details.pagerank_score > 0.0,
        "pagerank_score should be > 0, got {}",
        details.pagerank_score
    );
    assert!(
        details.betweenness > 0.0,
        "betweenness should be > 0, got {}",
        details.betweenness
    );
    assert!(
        details.score > 0.0,
        "composite score should be > 0, got {}",
        details.score
    );
    ...
    for i in 0..5 {
        ...
        assert!(
            details.score >= leaf_details.score,
...
fn benchmark_ba_graph_generation() {
    ...
    assert!(
        d_max > 6,
        "BA d_max should be > 6 due to preferential attachment, got {}",
        d_max
    );
}
...
fn benchmark_instrumented_search_correctness() {
    ...
    assert!(
        stats.edges_examined > 0 || stats.nodes_visited == stats.nodes_per_level[0],
...
fn benchmark_naive_vs_pruned_speedup() {
    ...
    assert!(
        ...
        result.nodes_visited_pruned,
        result.nodes_visited_naive
    );
    assert!(
        result.speedup >= 1.0,
        "Speedup should be >= 1, got {:.2}",
        result.speedup
    );
}
...
fn benchmark_beff_convergence() {
    ...
    if stats.nodes_per_level.len() >= 3 && stats.nodes_per_level[1] > 5 {
        assert!(
            beff < 5.0,
            "b_eff should be moderate for ER graph with 9 types, got {:.2}",
            beff
        );
    }
}
...
fn benchmark_ablation_time_window() {
    ...
    let (_, stats_with_window) = search_instrumented(&g, &h, Some((1_200_000, 1_800_000))).unwrap();

    // Window should reject additional edges
    assert!(
        stats_with_window.nodes_visited <= stats_no_window.nodes_visited,
...
fn benchmark_full_report() {
    ...
    let configs: Vec<(&str, GraphHunter)> = vec![
        (
            "ER-100",
            generate_erdos_renyi(100, 0.10, 1_000_000, 2_000_000, 42),
        ),
        (
            "ER-500",
            generate_erdos_renyi(500, 0.02, 1_000_000, 2_000_000, 42),
        ),
        (
            "ER-1K",
            generate_erdos_renyi(1000, 0.01, 1_000_000, 2_000_000, 42),
        ),
        (
            "ER-5K",
            generate_erdos_renyi(5000, 0.002, 1_000_000, 2_000_000, 42),
        ),
        (
            "BA-100",
            generate_barabasi_albert(100, 3, 1_000_000, 2_000_000, 42),
        ),
        (
            "BA-500",
            generate_barabasi_albert(500, 3, 1_000_000, 2_000_000, 42),
        ),
        (
            "BA-1K",
            generate_barabasi_albert(1000, 3, 1_000_000, 2_000_000, 42),
        ),
        (
            "BA-5K",
            generate_barabasi_albert(5000, 3, 1_000_000, 2_000_000, 42),
        ),
    ];

    println!(
        "{:<10} {:>6} {:>8} {:>6} {:>6} | {:>3} {:>6} {:>8} {:>10} {:>10} {:>6} | {:>6} {:>6} {:>6} {:>6}",
        "Graph",
        "n",
        "m",
        "d_max",
    ...
    for (label, g) in &configs {
        for l in [2, 3, 4, 5] {
            ...
            println!(
                "{:<10} {:>6} {:>8} {:>6} {:>6.1} | {:>3} {:>6} {:>8.2} {:>10} {:>10} {:>6.1} | {:>5.1} {:>5.1} {:>5.1} {:>6.2}",
                r.label,
                r.n_entities,
                r.n_relations,
                r.d_max,
...
fn benchmark_real_datasets() {
    ...
    if let Ok(data) = std::fs::read_to_string("../../demo_data/apt_attack_simulation.json") {
        ...
        let (n, m, d_max, d_avg, n_et, n_rt) = graph_params(&g);
        println!(
            "APT Simulation: n={}, m={}, d_max={}, d_avg={:.1}",
            n, m, d_max, d_avg
        );
        ...
        for l in [2, 3, 4, 5] {
            ...
            println!(
                "  L={}: results={}, time={:.3}ms, V_pruned={}, V_naive={}, speedup={:.1}x, b_eff={:.2}",
                l,
                r.results_found,
                r.time_us as f64 / 1000.0,
                r.nodes_visited_pruned,
    ...
    if let Ok(data) = std::fs::read_to_string("../../demo_data/sentinel_attack_simulation.json") {
        ...
        let (n, m, d_max, d_avg, n_et, n_rt) = graph_params(&g);
        println!(
            "\nSentinel Simulation: n={}, m={}, d_max={}, d_avg={:.1}",
            n, m, d_max, d_avg
        );
    ...
    if let Ok(data) = std::fs::read_to_string("../../demo_data/mordor_combined_attacks.json") {
        ...
        let (n, m, d_max, d_avg, n_et, n_rt) = graph_params(&g);
        println!(
            "\nMordor Combined: n={}, m={}, d_max={}, d_avg={:.1}",
            n, m, d_max, d_avg
        );
        ...
        for l in [2, 3, 4, 5] {
            ...
            let (rr_rel, rr_ent, rr_temp, _, _) = r.pruning_stats.rejection_rates();
            println!(
                "  L={}: results={}, time={:.3}ms, V_pruned={}, V_naive={}, speedup={:.1}x, b_eff={:.2}, prune: rel={:.0}% ent={:.0}% temp={:.0}%",
                l,
                r.results_found,
                r.time_us as f64 / 1000.0,
                r.nodes_visited_pruned,
...
fn build_3node_graph() -> GraphHunter {
    ...
    g.add_relation(Relation::new("A", "B", RelationType::Connect, 1000))
        .unwrap();
    g.add_relation(Relation::new("B", "C", RelationType::Execute, 2000))
        .unwrap();
    g.sort_edges_by_timestamp().unwrap();
...
fn anomaly_trivial_3node_graph() {
    ...
    assert!(
        (breakdown.entity_rarity - 0.667).abs() < 0.01,
        "ER = {}",
        breakdown.entity_rarity
    );

    // Edge Rarity: each edge pair seen once, max_pair=1, ln(1)=0
    // EdgeR(e) = 1 - ln(1)/ln(1) → log_max_pair_freq = ln(1) = 0, so edge_r = 0.0
    assert!(
        (breakdown.edge_rarity - 0.0).abs() < 0.01,
        "EdgeR = {}",
        breakdown.edge_rarity
    );
    ...
    assert!(
        (breakdown.neighborhood_concentration - 0.667).abs() < 0.01,
        "NC = {}",
        breakdown.neighborhood_concentration
    );

    // TN: first_seen A=1000, B=1000, C=2000, tau_min=1000, tau_max=2000
    // TN(A) = 0, TN(B) = 0, TN(C) = 1.0 → avg = 0.333
    assert!(
        (breakdown.temporal_novelty - 0.333).abs() < 0.01,
        "TN = {}",
        breakdown.temporal_novelty
    );
...
fn anomaly_symmetric_graph() {
    ...
    g.enable_anomaly_scoring(ScoringWeights::default());
    for i in 0..5 {
        g.add_entity(Entity::new(&format!("N{}", i), EntityType::Host))
            .unwrap();
    }
    ...
    assert!(
        breakdown.entity_rarity.abs() < 0.01,
        "ER should be ~0 for symmetric graph, got {}",
        breakdown.entity_rarity
    );
}
...
fn anomaly_single_neighbor_nc() {
    ...
    g.add_relation(Relation::new("center", "h1", RelationType::Connect, 1000))
        .unwrap();
    g.add_relation(Relation::new("center", "h2", RelationType::Connect, 2000))
        .unwrap();
    g.finalize_anomaly_scorer();
    ...
    let nc = scorer
        .score_path(&vec!["center".to_string()], &g)
        .1
        .neighborhood_concentration;
    assert!(
        (nc - 1.0).abs() < 0.01,
        "NC should be 1.0 for single-type neighbors, got {}",
        nc
    );
}
...
fn anomaly_uniform_nc() {
    ...
    assert!(
        nc < 0.01,
        "NC should be ~0 for uniform type distribution, got {}",
        nc
    );
}
...
fn anomaly_unique_edge() {
    ...
    for i in 0..5 {
        g.add_relation(Relation::new("A", "B", RelationType::Connect, 1000 + i))
            .unwrap();
    }
    ...
    assert!(
        (breakdown.edge_rarity - 1.0).abs() < 0.01,
        "EdgeR should be 1.0 for unique edge, got {}",
        breakdown.edge_rarity
    );
}
...
fn anomaly_temporal_novelty_ordering() {
    ...
    g.add_entity(Entity::new("new", EntityType::Process))
        .unwrap();
    g.add_relation(Relation::new("old", "mid", RelationType::Connect, 1000))
        .unwrap();
    g.add_relation(Relation::new("mid", "new", RelationType::Execute, 5000))
        .unwrap();
    g.finalize_anomaly_scorer();
    ...
    assert!(
        (breakdown.temporal_novelty - 1.0).abs() < 0.01,
        "TN should be ~1.0 for newest entity, got {}",
        breakdown.temporal_novelty
    );
    ...
    assert!(
        (breakdown_old.temporal_novelty - 0.0).abs() < 0.01,
        "TN should be ~0.0 for oldest entity, got {}",
        breakdown_old.temporal_novelty
    );
}
...
fn anomaly_weights_sum_correctly() {
    ...
    assert!(
        ...
        "Composite {} should equal weighted sum {}",
        score,
        expected
    );
}
...
fn anomaly_apt_simulation_scoring() {
    ...
    // Routine entities
    for i in 0..10 {
        g.add_entity(Entity::new(&format!("user{}", i), EntityType::User))
            .unwrap();
        g.add_entity(Entity::new(&format!("host{}", i), EntityType::Host))
            .unwrap();
    }
    g.add_entity(Entity::new("dc01", EntityType::Host)).unwrap();
    g.add_entity(Entity::new("attacker_proc", EntityType::Process))
        .unwrap();
    g.add_entity(Entity::new("malware.exe", EntityType::File))
        .unwrap();
    ...
    g.add_relation(Relation::new("user0", "dc01", RelationType::Auth, 9000))
        .unwrap();
    g.add_relation(Relation::new(
        "dc01",
        "attacker_proc",
...
fn anomaly_incremental_ingestion() {
    ...
    g.add_relation(Relation::new("C", "D", RelationType::Execute, 5000))
        .unwrap();
    g.finalize_anomaly_scorer();

    let scorer = g.anomaly_scorer.as_ref().unwrap();
    let (common_score, _) = scorer.score_path(&vec!["A".to_string(), "B".to_string()], &g);
    let (rare_score, _) = scorer.score_path(&vec!["C".to_string(), "D".to_string()], &g);

    // Rare path C→D should have higher anomaly score than common A→B
    assert!(
        rare_score > common_score,
        "Rare path ({}) should score higher than common ({})",
        rare_score,
        common_score
    );
}

// ── F6 — Scoring explicable sin GNN ──────────────────────────────
// These tests pin the "defendible claim" that GraphHunter produces a
// stable, fully explainable composite score without any ML/GNN
// dependency. Paired with docs/spec/scoring-v1.md, this is the
// evidence the paper's §scoring section cites.

/// DoD Fase 6: the shipping default profile must not activate the GNN
/// component. If someone flips this weight > 0 without a deliberate
/// code review + ADR, downstream users will unknowingly depend on a
/// model that isn't trained yet.
#[test]
fn scoring_v1_default_profile_has_zero_gnn_weight() {
    let w = ScoringWeights::default();
    assert_eq!(
        w.w5_gnn_threat, 0.0,
        "default profile must not activate GNN scoring (found w5_gnn_threat={})",
        w.w5_gnn_threat
    );
    // The other four must be strictly positive — a zero there would
    // silently disable a documented dimension.
    assert!(w.w1_entity_rarity > 0.0);
    assert!(w.w2_edge_rarity > 0.0);
    assert!(w.w3_neighborhood_conc > 0.0);
    assert!(w.w4_temporal_novelty > 0.0);
}

/// With `w_gnn=0` (default) and GNN scores injected, the composite
/// must NOT reflect those scores — the weight renormalization drops
/// the component entirely. This protects the "explainable-without-ML"
/// guarantee when GraphOS-APT caches are sideloaded.
#[test]
fn scoring_v1_default_profile_ignores_injected_gnn_scores() {
    let mut g = build_3node_graph();
    let path = vec!["A".to_string(), "B".to_string(), "C".to_string()];
    let (base_score, base_bd) = g.anomaly_scorer.as_ref().unwrap().score_path(&path, &g);

    // Inject maximum possible GNN scores for every node.
    let mut gnn: ahash::HashMap<String, f64> = ahash::HashMap::default();
    gnn.insert("A".to_string(), 1.0);
    gnn.insert("B".to_string(), 1.0);
    gnn.insert("C".to_string(), 1.0);
    g.anomaly_scorer.as_mut().unwrap().set_gnn_scores(gnn);

    let (after_score, after_bd) = g.anomaly_scorer.as_ref().unwrap().score_path(&path, &g);

    // Breakdown DOES reflect the GNN input (it's the raw component).
    assert!(
        (after_bd.gnn_threat - 1.0).abs() < 1e-10,
        "breakdown must surface the raw GNN component even when weighted to 0"
    );
    // Composite must NOT shift — w5=0 renormalizes the contribution out.
    assert!(
        (after_score - base_score).abs() < 1e-10,
        "default composite must ignore injected GNN scores; base={} after={}",
        base_score,
        after_score
    );
    // And the non-GNN breakdown components must be byte-identical.
    assert_eq!(base_bd.entity_rarity, after_bd.entity_rarity);
    assert_eq!(base_bd.edge_rarity, after_bd.edge_rarity);
    assert_eq!(
        base_bd.neighborhood_concentration,
        after_bd.neighborhood_concentration
    );
    assert_eq!(base_bd.temporal_novelty, after_bd.temporal_novelty);
}

/// Bit-for-bit reproducibility. A deterministic scorer is a prereq
/// for the paper's snapshot-based evidence (scores cited in §results
/// must be reproducible from the fixture alone). Re-building the same
/// graph twice and scoring the same path MUST yield identical f64s —
/// not "within epsilon".
#[test]
fn scoring_v1_is_bit_for_bit_reproducible() {
    let g1 = build_3node_graph();
    let g2 = build_3node_graph();
    let path = vec!["A".to_string(), "B".to_string(), "C".to_string()];

    let (s1, bd1) = g1.anomaly_scorer.as_ref().unwrap().score_path(&path, &g1);
    let (s2, bd2) = g2.anomaly_scorer.as_ref().unwrap().score_path(&path, &g2);

    assert_eq!(
        s1.to_bits(),
        s2.to_bits(),
        "composite score is not byte-identical across builds: {} vs {}",
        s1,
        s2
    );
    assert_eq!(bd1.entity_rarity.to_bits(), bd2.entity_rarity.to_bits());
    assert_eq!(bd1.edge_rarity.to_bits(), bd2.edge_rarity.to_bits());
    assert_eq!(
        bd1.neighborhood_concentration.to_bits(),
        bd2.neighborhood_concentration.to_bits()
    );
    assert_eq!(
        bd1.temporal_novelty.to_bits(),
        bd2.temporal_novelty.to_bits()
    );
    assert_eq!(bd1.gnn_threat.to_bits(), bd2.gnn_threat.to_bits());
}

/// Directional stability: a controlled change in the input (making
/// one entity rarer) must move the entity_rarity component in the
/// expected direction. This catches an accidental sign flip in the
/// `1 - ln(freq)/ln(max_freq)` formula that a range-only test would
/// miss.
#[test]
fn scoring_v1_rarer_entity_raises_entity_rarity() {
    // Baseline: all entities seen once → ER ≈ max.
    let mut g_base = GraphHunter::new();
    g_base.enable_anomaly_scoring(ScoringWeights::default());
    g_base
        .add_entity(Entity::new("rare", EntityType::User))
        .unwrap();
    g_base
        .add_entity(Entity::new("common", EntityType::Host))
        .unwrap();
    g_base
        .add_relation(Relation::new("rare", "common", RelationType::Auth, 1000))
        .unwrap();
    g_base.finalize_anomaly_scorer();
    let (_, bd_base) = g_base
        .anomaly_scorer
        .as_ref()
        .unwrap()
        .score_path(&vec!["rare".to_string()], &g_base);

    // Variant: `common` observed many extra times → `rare` is rarer
    // relative to max_freq, so ER(rare) must strictly increase.
    let mut g_var = GraphHunter::new();
    g_var.enable_anomaly_scoring(ScoringWeights::default());
    g_var
        .add_entity(Entity::new("rare", EntityType::User))
        .unwrap();
    g_var
        ...
        .unwrap();
    g_var
        .add_entity(Entity::new("other", EntityType::User))
        .unwrap();
    g_var
        .add_relation(Relation::new("rare", "common", RelationType::Auth, 1000))
        .unwrap();
    for _ in 0..20 {
        g_var
            .add_relation(Relation::new("other", "common", RelationType::Auth, 1500))
            .unwrap();
    }
    g_var.finalize_anomaly_scorer();
    let (_, bd_var) = g_var
        .anomaly_scorer
        .as_ref()
        .unwrap()
        .score_path(&vec!["rare".to_string()], &g_var);

    assert!(
        bd_var.entity_rarity > bd_base.entity_rarity,
        "inflating `common` frequency should raise ER(rare); base={} var={}",
        bd_base.entity_rarity,
        bd_var.entity_rarity
    );
}

// ── C2 — AMAC parity tests ───────────────────────────────────────────────
//
// The AMAC scheduler must produce the same SET of paths as the regular
// DFS for the same hypothesis on the same graph. Emission order is not
// part of the contract (`apply_aggregation` and `score_and_paginate_paths`
// both reorder), so the comparison is set-based.

fn amac_parity_graph_three_hop() -> GraphHunter {
    let mut g = GraphHunter::new();
    // Build a deliberately branchy graph so AMAC's K=8 contexts get
    // exercised: 4 starts × 3 mid-hops × 2 dest-hops = 24 candidate paths
    // before predicate filtering.
    for i in 0..4 {
        g.add_entity(Entity::new(format!("u{i}"), EntityType::User))
            .unwrap();
    }
    for i in 0..6 {
        g.add_entity(Entity::new(format!("h{i}"), EntityType::Host))
            .unwrap();
    }
    for i in 0..4 {
...
fn paths_as_set(paths: &[Vec<std::sync::Arc<str>>]) -> std::collections::HashSet<Vec<String>> {
    paths
        .iter()
...
#[test]
fn amac_parity_two_hop() {
    let g = amac_parity_graph_three_hop();
    let h = Hypothesis::new("auth-connect")
        .add_step(HypothesisStep::new(
            EntityType::User,
            RelationType::Auth,
            EntityType::Host,
    ...
    let (regular, _) = g.search_temporal_pattern(&h, None, Some(10_000)).unwrap();
    let (amac, _) = g.search_temporal_pattern_amac(&h, None, Some(10_000)).unwrap();

    let regular_set = paths_as_set(&regular);
    let amac_set = paths_as_set(&amac);
    assert_eq!(
        regular_set, amac_set,
        "AMAC/DFS divergence on 2-hop\n  only_dfs:  {:?}\n  only_amac: {:?}",
        regular_set.difference(&amac_set).collect::<Vec<_>>(),
        amac_set.difference(&regular_set).collect::<Vec<_>>(),
    );
}

#[test]
fn amac_parity_one_hop_with_wildcard() {
    let g = amac_parity_graph_three_hop();
    let h = Hypothesis::new("any-any").add_step(HypothesisStep::new(
        EntityType::Any,
        RelationType::Any,
        EntityType::Any,
    ));
    ...
    assert_eq!(paths_as_set(&regular), paths_as_set(&amac));
}

#[test]
fn amac_parity_palindromic_chain() {
    // Palindromic pattern triggers the symmetry-breaking filter. AMAC
    // must apply the filter identically to the regular DFS.
    let mut g = GraphHunter::new();
    for i in 0..5 {
        g.add_entity(Entity::new(format!("p{i}"), EntityType::Process))
            .unwrap();
    }
    let mut ts = 1000_i64;
    g.add_relation(Relation::new("p0", "p1", RelationType::Spawn, ts))
        .unwrap();
    ts += 1;
    g.add_relation(Relation::new("p1", "p2", RelationType::Spawn, ts))
        .unwrap();
    ts += 1;
    g.add_relation(Relation::new("p2", "p3", RelationType::Spawn, ts))
        .unwrap();
    ts += 1;
    g.add_relation(Relation::new("p3", "p4", RelationType::Spawn, ts))
        .unwrap();
    g.sort_edges_by_timestamp().unwrap();

    let h = Hypothesis::new("spawn-chain")
        .add_step(HypothesisStep::new(
            EntityType::Process,
            RelationType::Spawn,
            EntityType::Process,
...
#[test]
fn amac_parity_with_time_window() {
    let g = amac_parity_graph_three_hop();
    let h = Hypothesis::new("u-h").add_step(HypothesisStep::new(
        EntityType::User,
        RelationType::Auth,
        EntityType::Host,
    ));
    // Window covers half the auth events.
    let (regular, _) = g
        .search_temporal_pattern(&h, Some((1000, 1005)), Some(10_000))
        .unwrap();
    let (amac, _) = g
        .search_temporal_pattern_amac(&h, Some((1000, 1005)), Some(10_000))
        .unwrap();

    assert_eq!(paths_as_set(&regular), paths_as_set(&amac));
}

// ── D2-lite — semi-join parity tests ───────────────────────────────

#[test]
fn semijoin_parity_two_hop() {
    let g = amac_parity_graph_three_hop();
    let h = Hypothesis::new("u-h-ip")
        .add_step(HypothesisStep::new(
            EntityType::User,
            RelationType::Auth,
            EntityType::Host,
    ...
    let baseline = g.search_temporal_pattern(&h, None, Some(10_000)).unwrap().0;
    unsafe {
        std::env::set_var("GRAPHHUNTER_SEMIJOIN", "1");
    }
    let with_sj = g.search_temporal_pattern(&h, None, Some(10_000)).unwrap().0;
    unsafe {
        std::env::remove_var("GRAPHHUNTER_SEMIJOIN");
    }

    assert_eq!(paths_as_set(&baseline), paths_as_set(&with_sj));
}

#[test]
fn semijoin_drops_decoy_starts() {
    let mut g = GraphHunter::new();
    g.add_entity(Entity::new("u_full", EntityType::User))
        .unwrap();
    g.add_entity(Entity::new("u_decoy", EntityType::User))
        .unwrap();
    g.add_entity(Entity::new("h_full", EntityType::Host))
        .unwrap();
    g.add_entity(Entity::new("h_decoy", EntityType::Host))
        .unwrap();
    g.add_entity(Entity::new("ip", EntityType::IP)).unwrap();

    g.add_relation(Relation::new("u_full", "h_full", RelationType::Auth, 100))
        .unwrap();
    g.add_relation(Relation::new("h_full", "ip", RelationType::Connect, 110))
        .unwrap();
    g.add_relation(Relation::new("u_decoy", "h_decoy", RelationType::Auth, 100))
        .unwrap();
    g.sort_edges_by_timestamp().unwrap();
