...
pub fn export_subgraph_csv(
    ...
    for n in nodes {
        write_all(writer, format!("{},{},{:.2}\n", csv_escape(n.id), csv_escape(n.entity_type), n.score).as_bytes())?;
    }
    write_all(writer, b"\nsource,target,rel_type,timestamp\n")?;
    for e in edges {
        write_all(writer, format!("{},{},{},{}\n", csv_escape(e.source), csv_escape(e.target), csv_escape(e.rel_type), e.timestamp).as_bytes())?;
    }
    Ok(())
}
...
pub fn export_hunt_results_csv(
    ...
    write_all(writer, b"index,chain_summary,max_score,total_score,time_start,time_end,anomaly_score,path\n")?;
    for (i, sp) in paths.iter().enumerate() {
        let anomaly = sp.anomaly_score.map(|s| format!("{:.4}", s)).unwrap_or_default();
...
mod tests {
    ...
    fn test_subgraph_csv() {
        let nodes = vec![
            ExportSubgraphNode { id: "10.0.0.1", entity_type: "IP", score: 42.5 },
            ExportSubgraphNode { id: "host-a", entity_type: "Host", score: 10.0 },
        ];
        let edges = vec![
            ExportSubgraphEdge { source: "10.0.0.1", target: "host-a", rel_type: "Connect", timestamp: 1000 },
        ];
        let mut buf = Vec::new();
    ...
    fn test_subgraph_json() {
        let nodes = vec![
            ExportSubgraphNode { id: "n1", entity_type: "User", score: 5.0 },
        ];
        let edges = vec![];
        let mut buf = Vec::new();
    ...
    fn test_hunt_results_csv() {
        ...
        let output = String::from_utf8(buf).unwrap();
        assert!(output.contains("index,chain_summary"));
