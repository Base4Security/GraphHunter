...
fn report_graph_ram(g: &GraphHunter, label: &str) {
    ...
    let avg_entity_heap = if sampled > 0 { heap_sample / sampled } else { 0 };
    ...
    // edge_store: in-memory buffer of CompactRelation
    let edge_buf_bytes = g.edge_store.buffer_len() * std::mem::size_of::<CompactRelation>();
    println!(
        "  edge_store buffer ({} edges × {} B) = {}",
        g.edge_store.buffer_len(),
        std::mem::size_of::<CompactRelation>(),
        fmt_bytes(edge_buf_bytes)
    );
    ...
    let estimated_total =
        entities_total + edge_buf_bytes + reverse_total + type_index_est + interner_bytes;
    println!(
        "  ─────────────────────────────────────────────────────────────────────"
    );
